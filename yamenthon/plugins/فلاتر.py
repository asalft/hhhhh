...
from a_vun import Convert, zedub
...
async def memes(event):
    ...
    if gvarstatus("ZED_FONTS") is None:
        ZED_FONTS = "a_vun/helpers/styles/Terserah.ttf"
    else:
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لعـرض قائمـة خطـوط aSaL ",
        "الاستخـدام": "{tr}.الخطوط",
    },
)
async def lang(event):
    "لعـرض قائمـة خطـوط aSaL "
    input_str = event.pattern_match.group(1)
    if not input_str:
        await event.edit(f"**   ⃟⁞⃟⟢╎قائمـة خطـوط aSaL  هـي :-**\n**قم بنسخ اسم الخط ثم ارسل (.خط + اسم الخط)**\n\n{FONTS}")
        return
    else:
        return
...
async def lang(event):
    ...
    if not input_str:
        ...
        return
    if input_str == "عربي":
        arr = f"a_vun/helpers/styles/zarz.ttf"
    ...
    if input_str not in font_list:
        ...
        arg = f"a_vun/helpers/styles/{input_str}"
